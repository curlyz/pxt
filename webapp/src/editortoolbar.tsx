    ...
    renderCore() {
        ...
        return <div id="editortools" className="ui" role="region" aria-label={lf("Editor toolbar")}>
            ...
            <div style={{
                ...
                <EditorToolbarButton
                    color={`${this.state.isInteractive ? "green" : "gray"}`}
                    className={`medium download-button mobile tablet  ${downloadButtonClasses}`}
                    // className={`medium download-button mobile tablet hide  ${downloadButtonClasses}`}
                    onButtonClick={async () => {
                        console.log("Turn On/Off Interactive");
                        this.setState({ isInteractive: !this.state.isInteractive });
                        // (window as any).interactiveEnabled = !this.state.isInteractive;
                        await (window as any).setInteractive(!this.state.isInteractive)
                    }}
                    text={`Interactive: ${this.state.isInteractive ? "ON" : "OFF"}`}
                    view='computer'
                />
                {this.state.isInteractive &&
                    (
                        <div style={{ display: "flex" }}>
                            <span style={{ padding: 5 }}>Fast</span>
                            <input type="range" min={0} max={100} value={this.state.sliderPosition} onChange={async (e) => {
                                // console.log("Set Slow and Fast", e.target.value);
                                const value = e.target.value
                                await (window as any).setBlockDelay(parseInt(value))

                                this.setState({ sliderPosition: value })

                            }} />
                            <span style={{ padding: 5 }}>Slow</span>
                        </div>
                    )
                }
